...
class MostBeautifulItem{
    public int[] maximumBeauty(int[][] items, int[] queries) {
        ...
        int [] maxBeauty = new int [items.length + 1];

        Arrays.sort(items, (a, b) -> Integer.compare(a[0], b[0]));

        for( int i = 0; i < items.length; i++){
            maxBeauty[i+1] = Math.max(maxBeauty[i], items[i][1]);
        }
...
public class LeetCode2070 {
    public static void main(String[] args) {
        MostBeautifulItem mb = new MostBeautifulItem();
        int[][] items = { {1, 2}, {3, 2}, {2, 4}, {5, 6}, {3, 5} };
        int[] queries = { 1, 2, 3, 4, 5, 6 };

        // Call the method and get the result
        int[] result = mb.maximumBeauty(items, queries);
